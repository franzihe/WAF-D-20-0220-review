...
%DIF ADD revision_V2.tex   Wed Jun  2 18:01:19 2021
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\abstract{
	...
	Thus, simulations overestimate the amount of accumulated surface precipitation while underestimating snowfall in the vertical. These results highlight the need for further studies to constrain better the interplay between \DIFdelbegin \DIFdel{the evolution of snow }\DIFdelend \DIFaddbegin \DIFadd{snow evolution }\DIFaddend in the vertical and snowfall at the surface in numerical weather prediction models.
}
...
\DIFdelend \DIFaddbegin \DIFadd{for the relatively low-wind easterly events. }\DIFaddend In this work, we use these assumptions to compare surface snowfall accumulations in the OESR with DFAR measurements for a slightly different classification scheme for snowfall events than those used in the \citet{schirle_estimation_2019}. 
		...
		\DIFaddend Good agreement between retrieved and observed snowfall values at the surface \DIFdelbegin \DIFdel{is needed to provide }\DIFdelend \DIFaddbegin \DIFadd{suggests }\DIFaddend confidence in the retrieved SWC values aloft\DIFdelbegin \DIFdel{that we use later to evaluate snow water representation in MEPS, CTRL, and ICE-T}\DIFdelend \DIFaddbegin \DIFadd{. The accuracy of any snowfall retrieval scheme ultimately depends upon the selection of a particle model well-matched to the environmental conditions for a given snowfall regime \mbox{%DIFAUXCMD
...
. Current research focuses on using airplane in-situ microphysical observations to evaluate retrieved vertical profiles of snow water.  Such work, however, is beyond the scope of this paper}\DIFaddend .
	...
	\subsection{Operational Weather Forecast Model - MEPS}\label{sec:methodology:MEPS}
		...
		MEPS is the operational ensemble forecast system used at MET Norway \citep{frogner_convection-permitting_2019}. It is based on HARMONIE-AROME (version 40h1.1), a mesoscale non-hydrostatic, convection-permitting NWP model \citep{the_metcoop_team_metcoop_2017}\DIFaddbegin \DIFadd{, }\DIFaddend which, in turn, based on the model developed by Meteo-France, AROME \citep{seity_arome-france_2010,bengtsson_harmoniearome_2017}.
...
\section{Results}\label{sec:res}
	...
	\subsection{Retrieval validation}
		...
		The similarity between the retrieved surface snowfall amount from the OESR and the DFAR \DIFdelbegin \DIFdel{indicates that the OESR represents the vertical structure of the precipitation over the HTS well. Therefore, the OESR can }\DIFdelend \DIFaddbegin \DIFadd{suggests confidence in retrieved SWC values aloft, as discusses in Section \ref{sec:methodology}\ref{sec:methodology:oesr}. These values will }\DIFaddend be used as a reference to evaluate the vertical profiles of snow water from the CTRL and ICE-T simulations\DIFaddbegin \DIFadd{, }\DIFaddend as discussed in Section \ref{sec:res}\ref{sec:res:swc}.
	...
	\subsection{Validation of the vertical distribution of snowfall}\label{sec:res:swc}
		...
		Thus, the maximum mean SWC is assumed to correspond to the mean cloud base height. When comparing to the modeled SWC, both the CTRL and ICE-T simulations also capture a decrease in SWC at $\sim$600\,m above the surface (see Fig. \ref{fig:vert_swc}a), indicating that they \DIFdelbegin \DIFdel{are correctly capturing }\DIFdelend \DIFaddbegin \DIFadd{correctly account for }\DIFaddend the vertical evolution of hydrometeors as they fall to the surface.
