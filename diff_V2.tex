...
%DIF ADD revision_V2.tex   Tue Jun 22 19:53:55 2021
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
. Therefore, accurate observations of precipitation in the vertical are required to evaluate vertical model predictions.
    ...
    Quantitative \DIFdelbegin \DIFdel{estimates of snowfall }\DIFdelend \DIFaddbegin \DIFadd{snowfall estimates }\DIFaddend from radar-based remote sensing techniques rely on deriving a snowfall rate (S) from a radar reflectivity (Z). However, several studies have shown that the estimated value S depends critically upon the choice of microphysical assumptions \DIFaddbegin \DIFadd{such as particle models that are }\DIFaddend required for the retrieval \citep{kulie_utilizing_2009,friedrich_quantifying_2020}. Other studies \DIFdelbegin \DIFdel{have tried to incorporate }\DIFdelend \DIFaddbegin \DIFadd{incorporated }\DIFaddend scene-specific snowflake microphysical property information into the retrieval scheme to better match environmental conditions \citep{wood_microphysical_2015}. \DIFdelbegin \DIFdel{For example, }\DIFdelend \citet{cooper_variational_2017} used in-situ observations of snowflake microphysical properties from the Multi-Angle Snow Camera (MASC) to improve radar-based estimated snowfall at Barrow, Alaska. This study exploited an optimal estimation snowfall retrieval (OESR) approach that combined radar reflectivities, in-situ microphysical property estimates, and environmental information into a common retrieval framework to provide estimates of snowfall properties. 
    \DIFdelbegin \DIFdel{The preliminary evaluation suggested that the approach worked well for winter conditions at Barrow.
    }\DIFdelend 
	...
	snowfall accumulation using different assumptions of snow particle size distributions (PSD), habit, and fall speed from the instruments and evaluated \DIFaddbegin \DIFadd{them }\DIFaddend against DFAR observations. Total retrieved seasonal snowfall values agreed to within +20\,\% of snow gauge measurements for the two predominant snowfall regimes observed during the campaign, \DIFdelbegin \DIFdel{increasing }\DIFdelend \DIFaddbegin \DIFadd{suggesting preliminary }\DIFaddend confidence in the OESR approach. 

    \DIFdelbegin \DIFdel{NWP models with $\leq$4\,km resolution are now increasingly capable of better representation of terrain, which is critical for predicting orographic precipitation \mbox{%DIFAUXCMD
\citep{colle_59_2000, colle_1314_2005, garvert_1314_2005, schwartz_reproducing_2014}}\hspace{0pt}%DIFAUXCMD
. More importantly, the higher model resolution allows for representing small-scale phenomena such as convective dynamics \mbox{%DIFAUXCMD
\citep{gowan_validation_2018}}\hspace{0pt}%DIFAUXCMD
, thus enabling more accurate simulations of precipitation's magnitude and location. Improved microphysical parameterizations can influence the distribution of latent heat release, hence generating potential vorticity, which can affect storm evolution \mbox{%DIFAUXCMD
...
\section{Methodology}\label{sec:methodology}
	...
	\subsection{Snowfall regime analysis}\label{sec:methodology:snowfall_reg}
		...
		The wind regime was assigned for each minute observation when the surface temperature was less than 2\,$^{\circ}$C depending on the observed wind direction during that minute. However, to omit local topographically induced turbulence on the wind regime assignment, a given wind regime was only assigned if it lasted for more than 10 minutes. Thus\DIFaddbegin \DIFadd{, }\DIFaddend we allow wind shifts lasting less than 10 minutes to be assigned to the encompassing wind direction. Once we determined the wind regime to the minute data, the respective OESR was used to calculate the surface accumulated snowfall and the vertical snow water content (SWC). \DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{Due to instrument failures, we }\DIFaddend only considered days with continuous hourly observations (24\,h) in the analysis\DIFdelbegin \DIFdel{due to instrument failures}\DIFdelend . Furthermore, only days are used where the DFAR and OESR observed more than 0.25\,mm\,day$^{-1}$ precipitation to exclude measurement noise from the analysis.	
...
\citep{wood_microphysical_2015}}\hspace{0pt}%DIFAUXCMD
		...
		\DIFaddend Good agreement between retrieved and observed snowfall values at the surface \DIFdelbegin \DIFdel{is needed to provide }\DIFdelend \DIFaddbegin \DIFadd{suggests }\DIFaddend confidence in the retrieved SWC values aloft\DIFdelbegin \DIFdel{that we use later to evaluate snow water representation in MEPS, CTRL, and ICE-T}\DIFdelend \DIFaddbegin \DIFadd{. However, the accuracy of any snowfall retrieval scheme ultimately depends upon selecting a particle model well-matched to the environmental conditions for a given snowfall regime \mbox{%DIFAUXCMD
\citep{cooper_variational_2017}}\hspace{0pt}%DIFAUXCMD
. For example, at HTS, \mbox{%DIFAUXCMD
\citet{schirle_estimation_2019} }\hspace{0pt}%DIFAUXCMD
