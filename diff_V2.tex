...
%DIF DEL revision_V1.tex   Thu May 20 18:09:03 2021
%DIF ADD revision_V2.tex   Tue Jun 29 21:12:38 2021
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\abstract{
    In \DIFaddbegin \DIFadd{the }\DIFaddend winter, orographic precipitation falls as snow in the mid to high latitudes where it causes avalanches, affects local infrastructure, or leads to flooding during the spring thaw. \DIFdelbegin \DIFdel{Predicting snowfall amounts in mountainous regions is a challenge due to complex interactions between the local topography and dynamics. }\DIFdelend We present a technique to validate operational \DIFdelbegin \DIFdel{forecasts }\DIFdelend \DIFaddbegin \DIFadd{numerical weather prediction model simulations }\DIFaddend in complex terrain\DIFdelbegin \DIFdel{using a combination of remote sensing and in situ snowfall measurements. }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdel{A Micro Rain Radar and a Multi-Angle Snowflake Camera provide microphysical information exploited in an optimal estimation retrieval (OESR). The validation of snowfall forecasts focused on winter 2016-2017 when High-Latitude Measurement of Snowfall deployed these instruments at the Haukeliseter test sitein Norway. }\DIFdelend \DIFaddbegin \DIFadd{. The presented verification technique uses a combined DFAR and MRR retrieval approach to obtain surface snowfall accumulation and vertical profiles of snow water at the Haukelister test site, Norway. Surface and vertical snow observations are used to validate against model simulations from the Norwegian Meteorological Institute's operational forecast system and two simulations with adjusted cloud microphysics. This method can validate not only the surface snowfall accumulation but also the precipitation in the vertical.   }\DIFaddend \\ 
    Retrieved surface snowfall is validated against measurements conducted with a double-fence automated reference gauge. In comparison produces the OESR +10.9\,\% \DIFdelbegin \DIFdel{surface snowfall}\DIFdelend \DIFaddbegin \DIFadd{more surface snowfall than the DFAR}\DIFaddend . The predicted surface snowfall from the operational forecast model MetCoOp ensemble prediction system (MEPS) and two additional simulations with microphysical adjustments (CTRL and ICE-T) are overestimated at the surface with +41.0\,\%, +43.8\,\%, and +59.2\,\%, respectively. Simultaneously, the CTRL and ICE-T simulations underestimate the mean snow water path by -1071.4\,\% and -523.7\,\%, respectively. \DIFdelbegin \DIFdel{Thus, simulations overestimate the amount of accumulated surface precipitation while underestimating snowfall in the vertical }\DIFdelend \DIFaddbegin \\
	\DIFadd{The study shows that we would reach false conclusions only using surface accumulation or vertical snow water content profiles}\DIFaddend . These results highlight the need \DIFdelbegin \DIFdel{for further studies to constrain better the interplay between the evolution of snow in the vertical and snowfall at the surface }\DIFdelend \DIFaddbegin \DIFadd{to combine ground-based in-situ and vertically-profiling remote sensing instruments to identify biases }\DIFaddend in numerical weather prediction\DIFdelbegin \DIFdel{models}\DIFdelend .
}
...
\section{Introduction}
    \DIFdelbegin \DIFdel{Orographic precipitation is of great societal relevance, as more than one-sixth of the world's population receives water from glaciers and seasonal snowpacks \mbox{%DIFAUXCMD
\citep{barnett_potential_2005}}\hspace{0pt}%DIFAUXCMD
. As such, the topic has been a long-standing focus for both weather and climate research communities \mbox{%DIFAUXCMD
\citep{stoelinga_improvement_2003,schar_orographic_2005,ranzi_hydrological_2007}}\hspace{0pt}%DIFAUXCMD
.
    }%DIFDELCMD < 
...
\DIFdelend Forecasting precipitation quantitatively is challenging, especially in complex terrain where the evaluation of forecast models is difficult due to the sparse distribution of precipitation gauges \citep{barstad_evaluation_2005}. Furthermore, coarse-resolution numerical weather prediction (NWP) models are less skillful in simulating orographic precipitation, especially over narrow topography \citep{gowan_validation_2018}. This difficulty is unfortunate, as the forecasts of avalanches, glacier mass budgets, and flash floods depend on accurate models. 
	...
	\DIFaddbegin \DIFadd{NWP models with $\leq$4\,km resolution are now increasingly capable of better representation of terrain, which is critical for predicting orographic precipitation \mbox{%DIFAUXCMD
\citep{colle_59_2000, colle_1314_2005, garvert_1314_2005, schwartz_reproducing_2014}}\hspace{0pt}%DIFAUXCMD
. More importantly, the higher model resolution allows for representing small-scale phenomena such as convective dynamics \mbox{%DIFAUXCMD
\citep{gowan_validation_2018}}\hspace{0pt}%DIFAUXCMD
, thus enabling more accurate simulations of precipitation's magnitude and location. Improved microphysical parameterizations can influence the distribution of latent heat release, hence generating potential vorticity, which can affect storm evolution \mbox{%DIFAUXCMD
...
\citep{joos_influence_2012}}\hspace{0pt}%DIFAUXCMD
. Therefore, accurate observations of precipitation in the vertical are required to evaluate vertical model predictions.
}

    \DIFaddend Accurate surface observations of precipitation are required to validate snowfall predictions from models. However, measuring precipitation, especially snow, is a well-known challenge \citep{theriault_dependence_2012,wolff_measurements_2013,colli_improved_2015}.  Winter precipitation measurements can show \DIFdelbegin \DIFdel{a difference of }\DIFdelend more than 100\,\% \DIFaddbegin \DIFadd{difference }\DIFaddend between different observing networks where the exact values depend upon regional snowfall characteristics \citep{kochendorfer_analysis_2017}. At high wind speeds, precipitation loss can be severe. Additionally, blowing surface snow can enter the gauge and artificially enhance the reported precipitation \citep{nitu_iom_2018}. A double fence automated reference (DFAR) gauge provides a more accurate estimate of snow accumulation than single fence gauges during high wind speeds \citep{wolff_derivation_2015,kochendorfer_analysis_2017}. Nevertheless, the DFAR still has an under-catch of 10\,\% at wind speeds below 9\,m\,s$^{-1}$ and 20\,\% under-catch at wind speeds between 9\,m\,s$^{-1}$ and 20\,m\,s$^{-1}$ \citep{nitu_iom_2018}. Such uncertainties must be considered when using gauge observations to evaluate \DIFdelbegin \DIFdel{both }\DIFdelend model forecasts and remote sensing algorithm estimates of snowfall \citep{wolff_derivation_2015}. 

    Quantitative \DIFdelbegin \DIFdel{estimates of snowfall }\DIFdelend \DIFaddbegin \DIFadd{snowfall estimates }\DIFaddend from radar-based remote sensing techniques rely on deriving a snowfall rate (S) from a radar reflectivity (Z). However, \DIFdelbegin \DIFdel{several }\DIFdelend studies have shown that the estimated value S depends critically upon the choice of microphysical assumptions required for the \DIFdelbegin \DIFdel{retrieval \mbox{%DIFAUXCMD
\citep{kulie_utilizing_2009,friedrich_quantifying_2020}}\hspace{0pt}%DIFAUXCMD
. Other studies have tried to incorporate }\DIFdelend \DIFaddbegin \DIFadd{inversion \mbox{%DIFAUXCMD
\citep{kulie_utilizing_2009}}\hspace{0pt}%DIFAUXCMD
. Subsequent studies incorporated }\DIFaddend scene-specific snowflake microphysical property information into the retrieval scheme to better match environmental conditions \citep{wood_microphysical_2015}. \DIFdelbegin \DIFdel{For example, }\DIFdelend \citet{cooper_variational_2017} used in-situ observations of snowflake \DIFdelbegin \DIFdel{microphysical properties }\DIFdelend \DIFaddbegin \DIFadd{particle size distribution (PSD) and habit }\DIFaddend from the Multi-Angle Snow Camera (MASC) to improve radar-based estimated snowfall at Barrow, Alaska. This study exploited an optimal estimation snowfall retrieval (OESR) approach that combined radar reflectivities, in-situ microphysical property estimates, and environmental information into a common retrieval framework to provide estimates of snowfall \DIFdelbegin \DIFdel{properties. The preliminary evaluation suggested that the approach worked well for winter conditions at Barrow}\DIFdelend \DIFaddbegin \DIFadd{rate}\DIFaddend . 

    
    \citet{schirle_estimation_2019} \DIFdelbegin \DIFdel{applied }\DIFdelend \DIFaddbegin \DIFadd{modified }\DIFaddend the \citet{cooper_variational_2017} algorithm \DIFdelbegin \DIFdel{approach }\DIFdelend to the instrumentation and environmental conditions at the Haukeliseter Test Site (HTS) as part of the High-Latitude Measurement of Snowfall (HiLaMS) campaign. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{schirle_estimation_2019} }\hspace{0pt}%DIFAUXCMD
retrieved estimates of surface snowfall accumulation using different assumptions of snow particle size distributions (PSD), habit, and fall speed from the instruments and evaluated against DFAR observations. Total retrieved seasonal snowfall values agreed to within +20\,\% of snow gauge measurements for the two predominant snowfall regimes observed during the campaign, increasing confidence in the OESR approach. 
    }%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{NWP models with $\leq$4\,km resolution are now increasingly capable of better representation of terrain, which is critical for predicting orographic precipitation \mbox{%DIFAUXCMD
\citep{colle_59_2000, colle_1314_2005, garvert_1314_2005, schwartz_reproducing_2014}}\hspace{0pt}%DIFAUXCMD
. More importantly, the higher model resolution allows for representing small-scale phenomena such as convective dynamics \mbox{%DIFAUXCMD
\citep{gowan_validation_2018}}\hspace{0pt}%DIFAUXCMD
, thus enabling more accurate simulations of precipitation's magnitude and location. Improved microphysical parameterizations can influence the distribution of latent heat release, hence generating potential vorticity, which can affect storm evolution \mbox{%DIFAUXCMD
...
. Sensitivity tests varying the microphysical scheme of the Consortium for small-scale Modelling model showed that storm development depended on the correct vertical placement of the precipitation inside a modeled storm \mbox{%DIFAUXCMD
\citep{joos_influence_2012}}\hspace{0pt}%DIFAUXCMD
. Therefore, accurate observations of precipitation in the vertical are required to evaluate vertical model patterns}\DIFdelend \DIFaddbegin \DIFadd{This scheme is used in this work to estimate both surface snowfall rates and vertical profiles of snow water content (SWC)}\DIFaddend .
    ...
    Early versions of MEPS produced too much cloud ice with its default ICE3 cloud microphysics scheme. Thus, \citet{muller_arome-metcoop:_2017} \DIFdelbegin \DIFdel{made changes to }\DIFdelend \DIFaddbegin \DIFadd{changed }\DIFaddend the ICE3 microphysics scheme to improve the representation of fog, low-level, and cirrus clouds. More recently, \citet{engdahl_improving_2020} showed that even with the improvements, the ICE3 microphysics scheme depleted supercooled liquid water too quickly and produced a surplus of snow and graupel. For this reason, \citet{engdahl_improving_2020} introduced a series of changes to the ICE3 scheme based on the parameterization development by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{thompson_explicit_2004,thompson_explicit_2008}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{thompson_explicit_2004} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{thompson_explicit_2008}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The modified microphysics include updated ice nucleation\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend riming/accretion parameterization\DIFaddbegin \DIFadd{, }\DIFaddend and changes to the rain class PSD \citep{engdahl_improving_2020}. Idealized 1D experiments showed that the modified scheme prolonged the existence and produced higher amounts of supercooled liquid water \citep{engdahl_improving_2020}. Many studies with adjusted microphysical schemes, such as \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{liu_high-resolution_2011}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{liu_high-resolution_2011}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , have validated surface precipitation forecasts to observations \DIFdelbegin \DIFdel{, but not in the vertical .
    }\DIFdelend \DIFaddbegin \DIFadd{but not the vertical distribution of hydrometeors. A comparison of six microphysical parametrization schemes showed most parametrization schemes resemble each other in the upper troposphere but differ by a factor of three close to the surface. The \mbox{%DIFAUXCMD
\citet{lin_bulk_1983} }\hspace{0pt}%DIFAUXCMD
scheme (similar to ICE3) shows low snow and cloud water, yet high graupel amounts in the atmosphere. Combined with high surface precipitation amounts, this indicates a high precipitation efficiency. Conversely, the \mbox{%DIFAUXCMD
\citet{thompson_explicit_2008} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{morrison_impact_2009} }\hspace{0pt}%DIFAUXCMD
schemes have low graupel production. Still, the atmospheric snow and cloud liquid water contents are several times larger than in other simulations. 
}\DIFaddend 

    
    \DIFdelbegin \DIFdel{In this study, we demonstrate }\DIFdelend \DIFaddbegin \DIFadd{This study demonstrates }\DIFaddend the added value of validating snowfall in NWP models using surface and \DIFdelbegin \DIFdel{vertical observations }\DIFdelend \DIFaddbegin \DIFadd{OESR vertical profiles }\DIFaddend obtained by the advanced techniques of \citet{cooper_variational_2017} and \citet{schirle_estimation_2019} during winter 2016-2017. We use DFAR snow observations, radar-based snowfall retrievals \citep{cooper_variational_2017,schirle_estimation_2019}, and high-resolution forecast data from MEPS at the Haukeliseter test site (HTS). The instrumental setup at HTS is a unique opportunity to apply the radar snowfall retrieval by \citet{schirle_estimation_2019} and verify snowfall in the operational MEPS and sensitivity simulations with adjusted cloud microphysics conducted by \citet{engdahl_effects_2020}.

    We structure the remainder of this paper as follows: Section \ref{sec:methodology} describes the study location, observations, and methodology. Section \ref{sec:res} describes the snowfall regimes, followed by a validation of the OESR retrieved solid surface accumulated precipitation \DIFdelbegin \DIFdel{through a comparison }\DIFdelend \DIFaddbegin \DIFadd{compared }\DIFaddend to the DFAR. Next, we discuss the seasonal bias in the accumulated solid surface precipitation and the vertical snowfall distribution of the NWP model simulations (MEPS, CTRL, and ICE-T) relative to the DFAR and OESR. Finally, Section \ref{sec:conclusion} presents the conclusions.
...
\section{Methodology}\label{sec:methodology}
	...
	\subsection{Optimal Estimation Retrieval Algorithm}\label{sec:methodology:oesr}
		\citet{schirle_estimation_2019} describe the \DIFdelbegin \DIFdel{estimates of }\DIFdelend \DIFaddbegin \DIFadd{OESR technique used to estimate }\DIFaddend the vertical profile of snow properties\DIFdelbegin \DIFdel{derived from the OESR algorithm in detail. In general, the OESR uses a priori parameters from }\DIFdelend \DIFaddbegin \DIFadd{. Use of the flexible OESR approach allows a means to combine radar reflectivities, }\DIFaddend in-situ observations\DIFdelbegin \DIFdel{of snowflake microphysics to determine the PSD within }\DIFdelend \DIFaddbegin \DIFadd{, atmospheric temperature profiles, and a priori information into a common retrieval framework to provide an estimate of snowfall properties consistent with each. The scheme retrieves an exponential PSD for }\DIFaddend each range bin of the MRR \DIFdelbegin \DIFdel{by fitting an exponential size distribution. Using the exponential size distribution coupled with snow particle }\DIFdelend \DIFaddbegin \DIFadd{that then can be converted to SWC using particle model }\DIFaddend size-mass dimensional relationships\DIFdelbegin \DIFdel{allows an estimate of the SWC in each layer obtained by the OESR}\DIFdelend . The SWC is then transformed into \DIFdelbegin \DIFdel{snow }\DIFdelend \DIFaddbegin \DIFadd{snowfall }\DIFaddend water equivalent at the surface \DIFdelbegin \DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{through use of }\DIFaddend fall speed observations. 
\DIFdelbegin \DIFdel{Thus, the OESR allows for a range of possible retrieved snowfall values for a given MRR reflectivity value, which contrasts markedly with traditional Z-S relationship techniques \mbox{%DIFAUXCMD
\citep{friedrich_quantifying_2020}}\hspace{0pt}%DIFAUXCMD
.  
		}\DIFdelend 

		
		During the HiLaMS campaign, \citet{schirle_estimation_2019} \DIFdelbegin \DIFdel{used two OESRs}\DIFdelend \DIFaddbegin \DIFadd{explored the impact of different microphysical assumptions in the OESR on retrieved surface snowfall}\DIFaddend . They found \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{best-case }\DIFaddend differences between total seasonal retrieved surface accumulations and HTS DFAR observations of +\,\DIFdelbegin \DIFdel{16}\DIFdelend \DIFaddbegin \DIFadd{9}\DIFaddend \,\% and +\,\DIFdelbegin \DIFdel{9\% during westerly and easterly snowfall regimes}\DIFdelend \DIFaddbegin \DIFadd{16\,\% during easterly and westerly snowfall regimes, respectively}\DIFaddend . However, the combination of measurements that maximized retrieval performance differed for these \DIFdelbegin \DIFdel{regimes due to variable sampling biases for various environmental conditions . 
		}%DIFDELCMD < 

%DIFDELCMD < 		%%%
\DIFdel{Partially-rimed }\DIFdelend \DIFaddbegin \DIFadd{snowfall regimes. For the relatively low-wind easterly events, best OESR results used observations of PSD and fall speed from the MASC. For the high wind and snowfall westerly wind events, near-surface turbulence and blowing snow dictated the use of a temperature-PSD relationship and fall speeds from the MRR for best results. However, the most important assumption for the first-order accuracy of any snowfall retrieval scheme is the selection of a particle model that is well-matched to scene environmental conditions \mbox{%DIFAUXCMD
\citep{cooper_variational_2017,schirle_estimation_2019}}\hspace{0pt}%DIFAUXCMD
. At HTS, partially-rimed }\DIFaddend aggregates dominated the MASC images during both wind regimes (Fig. \ref{fig:snowflakes}). \DIFdelbegin \DIFdel{The OESR used the }\DIFdelend \DIFaddbegin \DIFadd{As such, \mbox{%DIFAUXCMD
\citet{schirle_estimation_2019} }\hspace{0pt}%DIFAUXCMD
used a }\DIFaddend snow particle aggregate model \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{wood_microphysical_2015} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend developed for the CloudSat mission \DIFdelbegin \DIFdel{due to its ability to produce the }\DIFdelend \DIFaddbegin \DIFadd{that produces a }\DIFaddend high reflectivity per unit mass relationship \DIFdelbegin \DIFdel{expected of }\DIFdelend \DIFaddbegin \DIFadd{consistent with }\DIFaddend aggregates entrained in high liquid water content aloft \DIFdelbegin \DIFdel{. For the high wind and snowfall westerly wind events, near-surface turbulence and blowing snow dictated the a priori of fall speed from the MRR and a parametrization relating particle size distribution (PSD) to temperature. For the relatively low-wind easterly events, the OESR used a priori information from in-situ observations of PSD and fall speed from the MASC}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{wood_microphysical_2015}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
		...
		\DIFaddend Good agreement between retrieved and observed snowfall values at the surface \DIFdelbegin \DIFdel{is needed to provide }\DIFdelend \DIFaddbegin \DIFadd{suggests }\DIFaddend confidence in the retrieved SWC values aloft\DIFdelbegin \DIFdel{that we use later to evaluate snow water representation in MEPS, CTRL, and ICE-T}\DIFdelend \DIFaddbegin \DIFadd{. At HTS, \mbox{%DIFAUXCMD
\citet{schirle_estimation_2019} }\hspace{0pt}%DIFAUXCMD
showed that using a rimed aggregate particle model produced retrieved snowfall values well matched the MET Norway DFAR observations. Given that surface snowfall characteristics are dictated by snowfall processes aloft, there should be a strong physical correlation between the aggregate particles at the surface and those the MRR measures above. The OESR scheme also uses a PSD-temperature relationship that allows PSD to change with height for more accurate in-cloud retrievals of SWC \mbox{%DIFAUXCMD
\citep{wood_estimation_2011}}\hspace{0pt}%DIFAUXCMD
. Current research focuses on using airplane in-situ microphysical observations to evaluate retrieved vertical profiles of snow water. Such work, however, is beyond the scope of this paper}\DIFaddend .
...
%DIFDELCMD < 		SWP =\\
%DIFDELCMD < 		&\quad = \int_{h = 300\,m}^{h = 3000\,m} SWC(h) dh  \\
%DIFDELCMD < 		&\quad = \frac{h_{3000\,m} - h_{400\,m}}{3} \left[ SWC(h_{400\,m}) + SWC(h_{3000\,m}) + 4 \cdot SWC \left( \frac{h_{400\,m} + h_{3000\,m}}{2}\right) \right], \qquad (g\,m^{-2})
%DIFDELCMD < 		\end{split}%%%
...
\section{Results}\label{sec:res}
	...
	\subsection{Retrieval validation}
		...
		Notably, the difference between the retrieved and measured precipitation could be due to \DIFaddbegin \DIFadd{the }\DIFaddend under-catch of the DFAR at high wind speeds \citep{theriault_impact_2015,nitu_iom_2018,colli_adjustments_2020}. \citet{nitu_iom_2018} showed that the DFAR has an under-catchment up to 10\,\% for wind speeds below 9\,m\,s$^{-1}$. Meanwhile, they estimated that the DFAR has an under-catch of 20\,\% during wind speed between 10\,m\,s$^{-1}$ and 20\,m\,s$^{-1}$. \DIFdelbegin \DIFdel{When }\DIFdelend \DIFaddbegin \DIFadd{Thus, when }\DIFaddend accounting for the influence of wind speed on the DFAR measurements, the difference between the DFAR and OESR surface accumulation becomes -6.8 \% and +9.2 \% for the westerly and easterly regimes, respectively (see Table \ref{tab:sfc_acc}). Thus, the DFAR under-catchment can explain the observed difference between the OESR and DFAR during westerly events. In contrast, during the easterly snowfall regime, accounting for potential under-catchment does not explain the difference between the OESR and DFAR. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{As a result, the }\DIFaddend cumulative difference between the DFAR and OESR\DIFaddbegin \DIFadd{, without correction, }\DIFaddend for the easterly snowfall regime over the entire \DIFaddbegin \DIFadd{season }\DIFaddend is only 11\,mm. 

		The similarity between the retrieved surface snowfall amount from the OESR and the DFAR \DIFdelbegin \DIFdel{indicates that the OESR represents the vertical structure of the precipitation over the HTS well. Therefore, the OESR can }\DIFdelend \DIFaddbegin \DIFadd{suggests confidence in retrieved SWC values aloft, as discusses in Section \ref{sec:methodology}\ref{sec:methodology:oesr}. In addition, these values will }\DIFaddend be used as a reference to evaluate the vertical profiles of snow water from the CTRL and ICE-T simulations\DIFaddbegin \DIFadd{, }\DIFaddend as discussed in Section \ref{sec:res}\ref{sec:res:swc}.

	\subsection{Validation of surface snowfall}\label{sec:res:season_sfc}
		Following the technique described in \DIFdelbegin \DIFdel{Sec. }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{sec:methodology}\ref{sec:methodology:MEPS_vali}, we compare the simulated accumulated surface snowfall of MEPS, CTRL, and ICE-T to the DFAR measurements over 27 days. Comparing MEPS, CTRL, and ICE-T reveals an overestimation of +41.0\,\%, +43.8\,\%, and +59.2\,\%, respectively (Table \ref{tab:sfc_acc}). \DIFdelbegin \DIFdel{Thus, MEPS slightly outperforms the simulations with more complex and adjusted microphysics. }\DIFdelend MEPS relies on an ensemble approach that \DIFdelbegin \DIFdel{more accurately predicts the dynamical evolution of precipitation events}\DIFdelend \DIFaddbegin \DIFadd{predicts precipitation events' dynamical evolution more accurately, }\DIFaddend leading to better performance \citep{frogner_convection-permitting_2019}. \DIFdelbegin \DIFdel{When comparing the simulations with advanced microphysics, the }\DIFdelend CTRL simulation outperforms ICE-T as ICE-T simulates more precipitation over HTS than the CTRL. The increase in simulated surface snowfall \DIFdelbegin \DIFdel{from CTRL to }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend ICE-T is likely related to increased \DIFdelbegin \DIFdel{snow growth}\DIFdelend \DIFaddbegin \DIFadd{generation of snow and reduced production of graupel}\DIFaddend , as snow \DIFdelbegin \DIFdel{remains in the atmosphere longer due to lower fall velocities }\DIFdelend \DIFaddbegin \DIFadd{has lower fall velocity }\DIFaddend than graupel, \DIFdelbegin \DIFdel{leading to more surface accumulation \mbox{%DIFAUXCMD
\citep{engdahl_effects_2020}}\hspace{0pt}%DIFAUXCMD
. The slower fall velocity of snowfall, in turn, leads to more simulated precipitation }\DIFdelend \DIFaddbegin \DIFadd{meaning the condensate can remain in the atmosphere longer, thereby be advected over further distances. \mbox{%DIFAUXCMD
\cite{engdahl_effects_2020} }\hspace{0pt}%DIFAUXCMD
showed that the precipitation pattern had changed with ICE-T, leading to less precipitation up-slope of the mountains and more }\DIFaddend on the lee side\DIFdelbegin \DIFdel{of the mountains compared to CTRL }\DIFdelend \DIFaddbegin \DIFadd{, where HTS is situated }\DIFaddend (see Fig. \DIFdelbegin \DIFdel{\ref{fig:sfc_WS_WD})through an eastward displacement in the transition from snowto graupel }\DIFdelend \DIFaddbegin \DIFadd{7 in \mbox{%DIFAUXCMD
\citet{engdahl_effects_2020}}\hspace{0pt}%DIFAUXCMD
). The total surface precipitation within the domain was reduced from ICE-T to CTRL \mbox{%DIFAUXCMD
\citep{engdahl_effects_2020}}\hspace{0pt}%DIFAUXCMD
, suggesting a lower precipitation efficiency. Likewise, \mbox{%DIFAUXCMD
\citet{liu_high-resolution_2011} }\hspace{0pt}%DIFAUXCMD
found that the \mbox{%DIFAUXCMD
\citet{lin_bulk_1983} }\hspace{0pt}%DIFAUXCMD
based scheme had less condensate in the atmosphere, combined with an excess of snowfall precipitation, and suggested a high precipitation efficiency due to a high fraction of fast-falling graupel.
}

		
		\DIFadd{\mbox{%DIFAUXCMD
\citet{engdahl_improving_2020} }\hspace{0pt}%DIFAUXCMD
changed the coefficients in the mass-diameter relation ($m(D) = aD^b$) and terminal fall velocity ($v(D) = cD^d (\rho_{00}/\rho_{dref})^{0.4}$) within ICE-T. All coefficients ($a, b, c, d$) were changed for snow, while only $c$ }\DIFaddend and \DIFdelbegin \DIFdel{an overall increase in snowfall over inland Norway \mbox{%DIFAUXCMD
\citep{engdahl_effects_2020}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{$d$ were changed for graupel by \mbox{%DIFAUXCMD
\citet{engdahl_improving_2020}}\hspace{0pt}%DIFAUXCMD
, which leads to graupel falling more slowly and snow falling slower when $D < 600\mu m$ and faster for $D > 600\mu m$. Yet, the size distribution of snow is shifted towards smaller particles, so the net fall speed of snow is reduced. 
		}

		\DIFadd{The shift in precipitation pattern is evident in the results}\DIFaddend . Indeed, when comparing the simulated accumulated surface snowfall by snowfall regime, the westerly snowfall regime events show an overestimation at the surface of +59.7\,\%, +59.3\,\%, +79.2\,\%, for MEPS, CTRL, ICE-T, respectively (see Table \ref{tab:sfc_acc}). 
...
\citep{koltzow_verification_2020}}\hspace{0pt}%DIFAUXCMD
	...
	\subsection{Validation of the vertical distribution of snowfall}\label{sec:res:swc}
		The vertical distribution of SWC, as retrieved by the MRR using the OESR, is used to validate CTRL and ICE-T to identify if \DIFdelbegin \DIFdel{the tendency for too much simulated accumulated surface precipitation corresponds to an overestimation of SWC in the vertical}\DIFdelend \DIFaddbegin \DIFadd{there is a connection between the overestimation of surface precipitation and the distribution of condensates in the atmosphere}\DIFaddend .
		...
		The microphysical adjustments within ICE-T lead to more SWC in the column above HTS, reducing the bias \DIFdelbegin \DIFdel{compared to }\DIFdelend \DIFaddbegin \DIFadd{than }\DIFaddend the mean SWC retrieved with the MRR. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{engdahl_effects_2020}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend Indeed, \citet{engdahl_effects_2020} demonstrated that the ICE-T scheme simulates an increase in the vertical distribution of snow and a slight reduction in graupel relative to CTRL above the HTS. Therefore, the ICE-T scheme allows \DIFdelbegin \DIFdel{for }\DIFdelend the modeled solid hydrometeors to remain in the cloud longer due to the slower fall velocity of snow than graupel, thereby increasing the SWC in the vertical. Nevertheless, ICE-T still simulates too much surface snowfall compared to the DFAR. 
		...
		Regardless, \DIFdelbegin \DIFdel{when comparing all event hours, }\DIFdelend the simulations overestimated the surface snowfall accumulation \DIFaddbegin \DIFadd{when comparing all event hours }\DIFaddend while significantly underestimating the SWP. The underestimation may be due to several factors, including a poor representation of the terrain in the model, an overzealous conversion of liquid to ice, and subsequent snow growth in the microphysics schemes. Indeed, the simulated location of the HTS was 51 meters higher and \DIFdelbegin \DIFdel{located }\DIFdelend closer to the barrier crest in the model domain. Thus, the modeled precipitation would be susceptible to more spillover precipitation at high wind speeds, as was observed. Additionally, with its \DIFdelbegin \DIFdel{slower }\DIFdelend \DIFaddbegin \DIFadd{reduced }\DIFaddend conversion of snow to graupel, ICE-T led to an increase in SWC and SWP over HTS. Therefore, it represented the vertical distribution of snowfall more accurately relative to the OESR. ICE-T gives more snowfall than the CTRL in the atmosphere and at the surface. Snow can stay longer in the atmosphere because of its lower fall velocity. Still, at the same time, the hydrometeors in ICE-T can have more mass but a lower diameter leading to more snow accumulation at the surface.  
\DIFdelbegin \DIFdel{Nevertheless, to improve regional models, }\DIFdelend \DIFaddbegin 

		\DIFadd{Nevertheless, }\DIFaddend an in-depth analysis of why these biases occur is required \DIFaddbegin \DIFadd{to improve regional models}\DIFaddend , especially in the vertical. \DIFaddbegin \DIFadd{We want to point out that this study only investigates the effects of changes to the ICE3 cloud microphysical scheme in the operational forecast model and the DFAR and MRR retrieval approach to validate forecast models. We are aware that differences between the model simulations and OESR could also be due to other aspects of the model, such as kinematic and thermodynamic structures. MET Norway continuously validates the operational model with radiosondes within the model domain. \mbox{%DIFAUXCMD
\citet{engdahl_effects_2020} }\hspace{0pt}%DIFAUXCMD
also showed good agreement between modeled and observed atmospheric temperature profiles for a drizzle test case.
		}

		\DIFadd{Furthermore, there exists a broad range of microphysical schemes. Most one-moment microphysical schemes calculate the particle mass explicitly and diagnose the number concentration based on the particle mass. While two-moment schemes, like \mbox{%DIFAUXCMD
\citet{thompson_explicit_2008} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{morrison_impact_2009}}\hspace{0pt}%DIFAUXCMD
, calculate the number concentration explicitly for some hydrometeors.  Higher-order schemes like the bin and three-moment scheme calculate the number concentration explicitly within bins of size intervals or simulate radar reflectivities. While the bin and three-moment schemes seem more applicable in this study, they are also computationally expensive to run operationally \mbox{%DIFAUXCMD
\citep{morrison_impact_2009}}\hspace{0pt}%DIFAUXCMD
. 
}\DIFaddend 

		Combining the surface and vertical validation presented in this study provides a new technique for validating \DIFaddbegin \DIFadd{operational forecast }\DIFaddend models with point measurements. \DIFdelbegin \DIFdel{Without the vertical information}\DIFdelend \DIFaddbegin \DIFadd{For example}\DIFaddend , one might assume that microphysical adjustments should reduce the model's surface precipitation amount at high wind speeds \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{without the vertical information }\DIFaddend However, corrections based on the surface observations alone would likely lead to \DIFdelbegin \DIFdel{a further underestimation of }\DIFdelend \DIFaddbegin \DIFadd{further underestimating }\DIFaddend the vertical SWC, leading to other errors in simulated parameters. Thus, using both vertical and surface observations during future model development should help when adjusting microphysics schemes. 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{a) DFAR, unprotected precipitation gauges and meteorological mast at HTS. The arrow indicates the westerly wind direction. The figure is adapted from }%DIFDELCMD < \protect%%%
...
. b) Additional instruments installed during HiLaMS during winter 2016-2017: Muli-Angle Snowflake Camera (MASC, left), Precipitation Imaging Package (middle), Micro Rain Radar (MRR, right).}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:instruments}
%DIFDELCMD < %%%
